...
try:
    ...
    with st.expander("Add a single document to the knowledge base", expanded=True):
        ...
        if uploaded_file is not None:
            ...
            if st.session_state.get('filename', '') != uploaded_file.name:
                ...
                me_without_ext = os.path.splitext(converted_filename)[0]
                lper.blob_client.upsert_blob_metadata(uploaded_file.name, {'converted': 'true', 'embeddings_added': 'true', 'converted_filename': parse.quote(filename_without_ext)})
    ...
    with st.expander("View documents in the knowledge base", expanded=False):
        ...
        try:
            data = llm_helper.get_all_documents(k=5000)
            if len(data) == 0:
